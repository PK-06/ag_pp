...
def init_db():
    ...
    # s'assure que la colonne notes existe avant une eventuelle reconstruction
    # de la table users ci-dessous
    ...
    # migration : le role 'user' est nouveau, une base existante peut avoir un
    # CHECK constraint sur users.role qui ne l'autorise pas encore -> on
    # reconstruit la table dans ce cas (SQLite ne permet pas d'ALTER un CHECK).
    row = db.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='users'").fetchone()
    if row and row[0] and "'user'" not in row[0]:
        db.executescript('''
            ...
            INSERT INTO users_new (id, username, password_hash, role, notes, created_at)
                SELECT id, username, password_hash, role, notes, created_at FROM users;
            DROP TABLE users;
    ...
    if 'owner_id' not in existing_cols:
        db.execute('ALTER TABLE rdv ADD COLUMN owner_id INTEGER REFERENCES users(id) ON DELETE CASCADE')
    db.commit()
...
@login_required
def segmented_view():
    return render_template('segmented.html')


@app.route('/api/all-users')
@login_required
def api_all_users_list():
    """Liste de tous les utilisateurs (y compris soi-meme), utilisee pour
    construire les colonnes de la vue segmentee."""
    db = get_db()
...
def find_overlap(db, date_debut, heure_debut, date_fin, heure_fin, exclude_ids=None, owner_id=None):
    ...
    Le conflit est cherche uniquement dans le "calendrier effectif" concerne :
    - owner_id=None (evenement partage) : conflits avec les autres evenements
      partages uniquement.
    - owner_id=<id> (evenement personnel) : conflits avec les evenements
      partages ET les evenements personnels de ce meme utilisateur.
    Les calendriers personnels de deux utilisateurs differents ne se
    conditionnent jamais mutuellement.
...
def api_rdv_list():
    ...
    # scope=mine (defaut) -> evenements partages + mes evenements personnels
    #   (utilise par l'agenda personnel)
    # scope=all -> tous les evenements, y compris les personnels de tout le
    #   monde, avec le nom du proprietaire (utilise par la vue segmentee)
    scope = request.args.get('scope', 'mine')
    if not start or not end:
        return jsonify({'error': 'start et end requis (YYYY-MM-DD)'}), 400
    db = get_db()
    query = '''SELECT rdv.*, users.username AS auteur, categories.nom AS categorie_nom,
                      owner.username AS owner_username
               FROM rdv
               LEFT JOIN users ON users.id = rdv.created_by
               LEFT JOIN categories ON categories.id = rdv.category_id
               LEFT JOIN users owner ON owner.id = rdv.owner_id
               WHERE date <= ? AND COALESCE(date_fin, date) >= ?'''
    params = [end, start]
    if scope == 'mine':
        query += ' AND (rdv.owner_id IS NULL OR rdv.owner_id = ?)'
...
def api_rdv_create():
    ...
    is_privileged = user and user['role'] in ('admin', 'editor')
    uneditable = int(data.get('uneditable', 0)) if is_admin else 0
    is_balisage = int(data.get('is_balisage', 0)) if is_admin else 0

    db = get_db()

    # owner_id : None = evenement partage (visible/modifiable par tous selon
    # role). Sinon = evenement personnel de l'utilisateur cible.
    # - admin/editor : peuvent viser n'importe quel utilisateur (depuis la vue
    #   segmentee), ou laisser vide pour un evenement partage.
    # - user : ne peut viser que lui-meme ou laisser vide (partage).
    owner_raw = data.get('owner_id')
    owner_id = None
    if owner_raw not in (None, '', 'null'):
        try:
            owner_id = int(owner_raw)
        except (TypeError, ValueError):
            return jsonify({'error': 'owner_id invalide'}), 400
        if is_privileged:
            exists = db.execute('SELECT 1 FROM users WHERE id = ?', (owner_id,)).fetchone()
            if not exists:
                return jsonify({'error': 'utilisateur cible introuvable'}), 404
        elif owner_id != session['user_id']:
            return jsonify({'error': 'acces refuse'}), 403
    ...
    if date_fin_raw:
        ...
        if not is_balisage:
            conflict = find_overlap(db, date_str, data['heure_debut'], date_fin_str, data['heure_fin'], owner_id=owner_id)
            if conflict:
        ...
        cur = db.execute(
            '''INSERT INTO rdv (date, date_fin, heure_debut, heure_fin, titre, description, couleur, category_id, created_by, uneditable, is_balisage, owner_id)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?)''',
            (date_str, date_fin_str, data['heure_debut'], data['heure_fin'], data['titre'],
             data.get('description', ''), data.get('couleur') or DEFAULT_COLOR,
             data.get('category_id') or None, session['user_id'], uneditable, is_balisage, owner_id)
        )
        db.commit()
    ...
    if not is_balisage:
        for ds in date_strs:
            conflict = find_overlap(db, ds, data['heure_debut'], ds, data['heure_fin'], owner_id=owner_id)
            if conflict:
                return jsonify({'error': f"Chevauchement avec \"{conflict['titre']}\" ({conflict['date']} {conflict['heure_debut']} -> {conflict['date_fin'] or conflict['date']} {conflict['heure_fin']})"}), 409
...
def api_rdv_update(rdv_id):
    ...
    # Un role 'user' ne peut modifier que le partage (owner_id NULL) ou son
    # propre calendrier personnel. admin/editor peuvent modifier tout le
    # monde (depuis la vue segmentee notamment).
    if not is_privileged and row['owner_id'] is not None and row['owner_id'] != session['user_id']:
        return jsonify({'error': 'acces refuse'}), 403

    if row['uneditable'] and not is_admin:
    ...
    # Reassignation du proprietaire (partage <-> personnel de tel utilisateur)
    ...
    if 'owner_id' in data:
        ...
        if owner_raw in (None, '', 'null'):
            ...
            try:
                new_owner_id = int(owner_raw)
            except (TypeError, ValueError):
                return jsonify({'error': 'owner_id invalide'}), 400
            if is_privileged:
                exists = db.execute('SELECT 1 FROM users WHERE id = ?', (new_owner_id,)).fetchone()
                if not exists:
                    return jsonify({'error': 'utilisateur cible introuvable'}), 404
            elif new_owner_id != session['user_id']:
                return jsonify({'error': 'acces refuse'}), 403
        fields['owner_id'] = new_owner_id
    ...
    if not new_is_balisage:
        conflict = find_overlap(db, new_date, new_heure_debut, date_end_check, new_heure_fin, exclude_ids=[rdv_id], owner_id=new_owner_id)
        if conflict:
            return jsonify({'error': f"Chevauchement avec \"{conflict['titre']}\" ({conflict['date']} {conflict['heure_debut']} -> {conflict['date_fin'] or conflict['date']} {conflict['heure_fin']})"}), 409
...
def api_rdv_delete(rdv_id):
    ...
    if not is_privileged and row['owner_id'] is not None and row['owner_id'] != session['user_id']:
        return jsonify({'error': 'acces refuse'}), 403
    if row['uneditable'] and user['role'] != 'admin':
        return jsonify({'error': 'Cet evenement est verrouille par un administrateur'}), 403
...
def admin_users_create():
    ...
    db = get_db()
    if not username or not password or role not in ('editor', 'viewer', 'user'):
        users = db.execute('SELECT id, username, role, created_at FROM users ORDER BY id').fetchall()
        return render_template('users.html', users=users, error='Champs invalides (role: editor, user ou viewer uniquement)')
    try:
        db.execute(
            'INSERT INTO users (username, password_hash, role) VALUES (?,?,?)',
